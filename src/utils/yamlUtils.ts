...
export function generateFaultYaml(fault: FaultPattern): string {
  const faultDoc = {
    ...
    propagation_chain: (fault.propagation_chain || []).map((p) => ({
      from: p.from,
      to: p.to,
      from_device_type: p.from_device_type || undefined,
      from_device_name: p.from_device_name || undefined,
      from_symptom_name: p.from_symptom_name || undefined,
      to_device_type: p.to_device_type || undefined,
      to_device_name: p.to_device_name || undefined,
      to_symptom_name: p.to_symptom_name || undefined,
